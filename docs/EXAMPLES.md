...
| First-report STALL | The library never retries (2.0.0). The multi-profile examples report a STALL once and send again on their next loop iteration, because a refused report stays pending; `examples/c/verify/verify_common.h` resends explicitly up to 20 times, 1 ms apart. **[Caller example policy based on implementation observation, `SOURCE_CONFLICTS.md` T-03]** |
...
| Keyboard `0x04` through `0x65` | Keyboard a/A through Keyboard Application comes from HUT 1.7 §10; the Usage interval is the only caller choice, since every declared key is a one-bit Variable field. **[HUT 1.7 definition; caller example policy]** |
