...
deterministic fake-libusb tests, including ThreadSanitizer runs. Every row
below is **[unverified on hardware]**; record the target, OS/backend, and
result before any release wording relies on it.

| # | Hypothesis | Status |
|---|---|---|
| 1 | In current USB mode (before request 53) the target accepts requests 54-57 and the events reach input (T-07). | `Not run` [unverified on hardware] |
| 2 | After `aoahid_accessory_start`, the target re-enumerates as `18d1:2d00`, or `18d1:2d01` with USB debugging enabled, at the same bus and port path. | `Not run` [unverified on hardware] |
| 3 | In accessory mode, requests 54-57 register the HID and deliver input exactly as in current USB mode. | `Not run` [unverified on hardware] |
...
| 6 | On Windows, `2d00`/`2d01` binds a WinUSB-compatible driver so libusb can open it. | `Not run` [unverified on hardware] |
