...
|---|---|---|
| Touch contacts | 1 through 16 | Application-oriented policy aligned with the project's selected `MotionEvent` scope; shared identically by Touchscreen and Touchpad, whose contact fields both validate through the same `TouchFields` representation. It is not a HID or kernel maximum and remains **[Unverified on hardware]**. |
| Contacts per report | 1 through configured maximum | Without multi-packet mode, every configured contact fits one report. With multi-packet mode, first packet carries total count and continuations carry zero. Applies to both Touchscreen and Touchpad. |
...
| Touchpad `button_count` | 0 through 65535 | Zero is an explicitly valid buttonless/clickpad declaration; the ceiling matches the one-bit-per-Usage Button Page range `aoahid_touchpad_button` addresses one-based. Shares the same one-based, edge-guarded button state machine as Mouse and Gamepad buttons. |
