...
>
> Keyboard: the library maintains the key array or bitmap. A seventh simultaneous key in a 6KRO layout fills all six slots with `ErrorRollOver` (0x01) as the specification requires and recovers automatically. Modifier usages 0xE0-0xE7 are routed to the modifier byte and never into the array. All keys and modifiers are released before the node is unregistered, because a stuck modifier on Android is invisible and miserable to debug. No LED or Output surface exists. Rollover form, declared usage range, report IDs, and whether a consumer report shares the node are caller-supplied.
>
