...
These are protocol-level strategies. The current `libaoahid` runtime implements
only §4.1; §4.2 is retained as source material for protocol comparison.
...
> **libaoahid scope note:** This subsection documents the AOA protocol for
> comparison and auditing. The current `libaoahid` runtime does not implement
> this transition: it never sends requests 52, 53, or 58 and never waits for
> re-enumeration. `AOAHID_START_ACCESSORY_MODE` and the former Mode-B option
> members remain only as ABI tombstones; selecting the mode returns
> `AOAHID_ERR_UNSUPPORTED` before USB I/O. This **[Guide policy]** does not turn
> Mode A into a universal target guarantee; `SOURCE_CONFLICTS.md` T-07 records
> the exact implementation observations and hardware-verification boundary.
...
Place public `poll()` on the `Context` or transport runtime. If compatibility requires a `poll` member on `Device`, document that it may execute callbacks for the entire shared Context, not only that device. Do not stop the event pump merely because one device has no local in-flight transfer; doing so can starve another device on the same Context. A `Device` profile-state/send API may be single-threaded without requiring a separate Context per thread.
...
- A separate Mode-B host implementation (not the current `libaoahid` runtime)
  can send manufacturer/model/version plus `START` to create an Accessory
  interface. Such an implementation must state that Android application
  discovery or association UI may appear.
...
- Setup packets and short-transfer tests pass for current-mode requests 51 and
  54-57; the retained Mode-B token and tombstone inputs are rejected before USB
  I/O, and no runtime path sends requests 52, 53, or 58.
