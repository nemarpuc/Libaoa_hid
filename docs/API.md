...
| Gamepad/Joystick | `aoahid_gamepad_button`, `_set_axis`; `aoahid_dpad(node, up, down, right, left)` |
| Touchscreen/Touchpad | `aoahid_touch(node, contact_id, down, x, y, extra)`; touchpad button call |
...
|---|---|---|
| Keyboard Array/bitmap | Modifier Usages are routed to their separate bits. Array nonmodifiers are packed; exceeding `array_length` puts ErrorRollOver in every Array cell, and returning within capacity restores ordinary selectors. Bitmap mode maps each declared Usage to one bit and synthesizes no Array rollover. Duplicate edges are suppressed and an opposite edge waits for the first accepted report. `aoahid_kbd` has no release-all call; the caller releases each Usage it pressed. | Array/bitmap form, Usage interval and width, Array length or bitmap size, Report ID, and acknowledgement required by the bitmap/HID Appendix C conflict. |
...
| Gamepad/Joystick | `aoahid_dpad` maps `(up, down, right, left)` to Hat values Up `0`, Up-right `1`, Right `2`, Down-right `3`, Down `4`, Down-left `5`, Left `6`, Up-left `7`, and no direction to Null `15`. Opposite pairs return `AOAHID_ERR_PARAM` without changing Hat state. In `AOAHID_DPAD_BUTTONS` mode the four booleans remain independent OOC bits, including simultaneous opposites. Buttons and direction changes retain accepted edges; close restores explicit axis/D-pad neutrals. `application = AOAHID_CONTROLLER_JOYSTICK` reuses this same state machine under the Joystick Application Collection. | Application collection, axes, ranges, widths, neutrals, ordinary buttons, D-pad representation, Report ID, and target mappings. Only Game Pad + canonical Hat + a contiguous Button range from `1` with at least five fields is a portable candidate; raw/no-D-pad forms and Joystick remain conditional. |
| Touchscreen/Touchpad | `aoahid_touch(node, contact_id, down, x, y, extra)` auto-detects placement (a not-yet-active `contact_id`) versus movement (an already-active one) from `down=1`, and lift from `down=0`; Contact Count is computed from the records in the frame, only the first packet carries the total, and continuation packets carry zero. An Up record retains the Contact ID and emits one Tip=0 record before reuse. Tip=1 pressure is floored to one when present. Enabled Scan Time is computed at first-packet submission as elapsed steady-clock time in 100-microsecond ticks, reused for continuation packets, wrapped to the declared counter domain, and restarted at zero after inactivity. Contact, packet, and touchpad-button conflicts return `AOAHID_ERR_BUSY`. Setting `touchpad_button_count` above zero reuses this exact state machine under the Touch Pad Application Collection. | Coordinate, Contact ID/Count/Scan Time ranges and widths, maximum contacts, contacts per report, optional fields, Report ID, `touchpad_button_count`, and gesture/classification policy. |
